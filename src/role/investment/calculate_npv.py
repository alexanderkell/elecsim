...
#
# def get_plant_with_max_npv(model, difference_in_discount_rate, look_back_years):
#     npv_data = compare_npv(model, difference_in_discount_rate, look_back_years)
...
#         power_plant_name = "{}-{}-{}".format(plant_type, model.year_number,randint(1,99999999))
#         power_plant = create_power_plant(name=power_plant_name, start_date=model.year_number, capacity=capacity, simplified_type=plant_type)
#         return power_plant
#     else:
#         return None
#
# def get_affordable_plant_generator(model, difference_in_discount_rate, look_back_years):
#     npv_rows = get_positive_npv_plants(model, difference_in_discount_rate, look_back_years)
...
# def get_positive_npv_plants(model, difference_in_discount_rate, look_back_years):
#     npv_data = compare_npv(model, difference_in_discount_rate, look_back_years)
#     npv_positive = npv_data[npv_data.npv_per_mw > 0]
#     return npv_positive
#
# def compare_npv(model, difference_in_discount_rate, look_back_years):
#     cost_list = []
#
...
#             npv = calculate_npv(plant_row.Type, plant_row.Plant_Size, model, difference_in_discount_rate, look_back_years)
...
#
#
# def calculate_npv(plant_type, plant_size, model, difference_in_discount_rate, look_back_years):
#     # Forecast segment prices
#     logger.info("Calculating NPV")
#     forecasted_segment_prices = _get_load_duration_price_predictions(model, difference_in_discount_rate, look_back_years)
#
#     logger.debug("Load duration prices: {}".format(forecasted_segment_prices))
#
#     power_plant = create_power_plant("estimate_variable", model.year_number, plant_type, plant_size)
#
#     # Forecast marginal costs
#     short_run_marginal_cost = _get_predicted_marginal_cost(power_plant, model, difference_in_discount_rate, look_back_years)
#     logger.debug("short run marginal cost: {}".format(short_run_marginal_cost))
#
#     forecasted_segment_prices = _clean_segment_prices(forecasted_segment_prices)
#     logger.debug("forecasted_segment_prices: \n {}".format(forecasted_segment_prices))
#
#     _get_profit_per_mwh(forecasted_segment_prices, short_run_marginal_cost)
#
#     _get_profit_per_segment(forecasted_segment_prices, power_plant=power_plant)
#
#     _get_total_hours_to_run(forecasted_segment_prices, power_plant)
#
#     _get_total_yearly_income(forecasted_segment_prices, power_plant)
#
...
#     yearly_capital_cost = _get_capital_outflow(power_plant)
#
#     logger.debug("yearly_capital_cost: {}".format(yearly_capital_cost))
#
#     yearly_operating_cash_flow = _calculate_yearly_operation_cashflow(power_plant, short_run_marginal_cost, total_running_hours,
...
#         weighted_average_cost_capital = nuclear_wacc + difference_in_discount_rate
#     else:
#         weighted_average_cost_capital = non_nuclear_wacc + difference_in_discount_rate
#
#     # for i in range(power_plant.pre_dev_period+power_plant.construction_period+power_plant.operating_period):
#     # cash_flow_wacc = [cash_flow/(1+weighted_average_cost_capital)**counter for counter, cash_flow in enumerate(total_cash_flow,1)]
#
#     # cash_flow_wacc = [cash_flow/(1+weighted_average_cost_capital)**counter for counter, cash_flow in enumerate(total_cash_flow,1)]
#
#
#     # logger.debug("cash_flow_wacc: {}".format(cash_flow_wacc))
#     logger.debug("discount rate: {}".format(weighted_average_cost_capital))
#
#     npv_power_plant = npv(weighted_average_cost_capital, total_cash_flow)
#
...
#     return NPVp
#
# def _get_yearly_profit_per_mwh(power_plant, total_running_hours, yearly_cash_flow):
#     yearly_cash_flow_per_mwh = yearly_cash_flow / (total_running_hours * power_plant.capacity_mw)
#     return yearly_cash_flow_per_mwh
#
# def _calculate_yearly_operation_cashflow(power_plant, short_run_marginal_cost, total_running_hours, total_yearly_income,
#                                          yearly_capital_cost):
...
#     return result
#
# def _get_capital_outflow(power_plant):
...
#
# def _get_total_yearly_income(forecasted_segment_prices, power_plant):
#     forecasted_segment_prices['total_income'] = forecasted_segment_prices.apply(
#         lambda x: _income(x, power_plant.capacity_mw), axis=1)
#
# def _get_total_hours_to_run(forecasted_segment_prices, power_plant):
#     forecasted_segment_prices['_total_running_hours'] = forecasted_segment_prices.apply(
#         lambda x: _total_running_hours(x, power_plant), axis=1)
#
# def _get_profit_per_segment(forecasted_segment_prices, power_plant):
#     forecasted_segment_prices['_total_profit_per_segment'] = forecasted_segment_prices.apply(
#         lambda x: _total_profit_per_segment(x, power_plant.capacity_mw), axis=1)
#
# def _get_profit_per_mwh(forecasted_segment_prices, short_run_marginal_cost):
#     forecasted_segment_prices['predicted_profit_per_mwh'] = forecasted_segment_prices[
#                                                                 'accepted_price'] - short_run_marginal_cost
#
# def _clean_segment_prices(forecasted_segment_prices):
...
#
# def _get_predicted_marginal_cost(power_plant, model, difference_in_discount_rate, look_back_years):
#     market_data = LatestMarketData(model=model)
...
#
# def _get_load_duration_price_predictions(model, difference_in_discount_rate, look_back_years):
#     load_duration_prices = LoadDurationPrices(model=model)
#     forecasted_segment_prices = load_duration_prices.get_load_curve_price_predictions(
#         reference_year=model.year_number + 1, look_back_years=look_back_years)
#     return forecasted_segment_prices
#
