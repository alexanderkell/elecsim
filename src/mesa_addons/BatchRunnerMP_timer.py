            ...
            for i, kwargs in enumerate(all_kwargs):
                ...
                for _ in range(self.iterations):
                    ...
                    job_queue.append((self.pool.uimap(self.run_iteration,
                                                     (kwargs,),
                                                     (param_values,),
                                                     (next(run_count),)), param_values))

            # empty the queue
            results = []
            for task, param_values in job_queue:
                start = process_time()
                for model_vars, agent_vars in list(task):
                    results.append((model_vars, agent_vars))
                end = process_time()
                total_time = end-start
                log.warning("total_time:{}, carbon_tax:{},  total capacity:{}".format(total_time, param_values[0][0], param_values[1].Capacity.sum()))
