...
#    ray.init(num_cpus=4)
    ...
    for scale_val in range(16,20):
        ...
        # dist_object = dist_class(size=50000).tolist()
        # dist_object =
    # print(len(result_distributions_object))
    # for resultant_dists in result_distributions_object:
    #     print(resultant_dists)

        # dist_class = eval(list(result_distributions_object[resultant_dists].fitted_param.keys()) + ".rvs")
        ...
        resultant_dists = "normal_loc_0_scale_{}".format(scale_val)
        # print(dist_object))
        ...
        # Parallel(n_jobs=multiprocessing.cpu_count()-1)(delayed(run_world)(carbon_list, resultant_dists, dist_object) for _ in range(0, 100))

        # Parallel(n_jobs=7)(delayed(run_world)(carbon_list, resultant_dists, dist_object) for i in tqdm(range(0, 100)))

        for _ in range(100):
            output = run_world.remote(carbon_list, resultant_dists, dist_object)
