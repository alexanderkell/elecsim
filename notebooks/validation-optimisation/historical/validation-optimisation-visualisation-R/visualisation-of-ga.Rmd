...
best_params %>% transform(y=get_line(best_c, best_m), x=get_x())  %>% ggplot() + stat_smooth(data=pdc_example, aes(x=segment_demand, y=accepted_price, color="Simulated Fit (2018)"), method="lm") + geom_point(data=pdc_example, aes(x=segment_demand, y=accepted_price, color="Price Duration Curve (2018)")) + geom_line(aes(x=x.1, y=x, color="Predicted")) + xlab("Demand (MW)") + ylab("Accepted Price (£/MWh)") + scale_color_manual(name = "Data",  values = c("Simulated Fit (2018)" = "blue", "Predicted" = "red", 'Price Duration Curve (2018)'='black'))  + theme_bw()

ggsave("~/Documents/PhD/Projects/10. ELECSIM/notebooks/validation-optimisation/figures/results/best_run_price_dur_curve.pdf", dpi=500, height=5, width=5)
```
